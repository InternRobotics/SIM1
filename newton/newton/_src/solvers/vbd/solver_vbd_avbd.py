...
# AVBD (Augmented Vertex Block Descent) parameters for strain limiting
# Reference: AVBD SIGGRAPH 2025
#
# The AVBD approach uses one-sided hard constraints for edge lengths:
# - Only prevents stretching (elongation), NOT compression
# - Allowing compression is KEY for cloth to form natural wrinkles
#
# Recommended parameter combinations for realistic cloth:
# - AVBD edge constraint:
#   - limit_ratio: 0.005 (0.5%) - small elasticity for solver convergence
#   - initial_stiffness: 1000.0 - AVBD auto-accumulates force
# - Isotropic SNH (for soft shear):
#   - mu: 20.0 ~ 100.0 (very small) - smaller = softer, more silk-like
#   - lambda: 1000.0+ - maintains area, prevents penetration
# - Bending: use small stiffness (e.g., 0.01) for rich fine wrinkles
#
AVBD_ALPHA = 0.95  # Constraint regularization coefficient (0.0~1.0), prevents explosive error correction
AVBD_BETA = 10.0  # Stiffness growth rate
AVBD_INITIAL_STIFFNESS = 1000.0  # Initial stiffness for strain limit constraints
AVBD_MAX_STIFFNESS = 1.0e9  # Maximum stiffness limit
...
@wp.func
def evaluate_SNH_force_hessian(
    face: int,
    v_order: int,
    pos: wp.array(dtype=wp.vec3),
    pos_prev: wp.array(dtype=wp.vec3),
    ...
    # Stable Neo-Hookean Energy for 2D Surface Elements (Shells)
    # Reference: Smith et al., "Dynamic Deformables"
    # Psi = 0.5 * mu * (Ic - 2) - mu * (J - 1) + 0.5 * lambda * (J - 1)^2
    ...
    # Cache Inverse Rest Pose (DmInv)
    DmInv00 = tri_pose[0, 0]
    DmInv01 = tri_pose[0, 1]
    DmInv10 = tri_pose[1, 0]
    DmInv11 = tri_pose[1, 1]

    # 2. Compute Deformation Gradient F = [f0, f1]
    f0 = x01 * DmInv00 + x02 * DmInv10
    f1 = x01 * DmInv01 + x02 * DmInv11

    # -------------------------------------------------------------
    # ELASTICITY: Stable Neo-Hookean
    # -------------------------------------------------------------
    
    # J = ||f0 x f1|| (Area stretch ratio)
    n_vec = wp.cross(f0, f1)
    J = wp.length(n_vec)
    ...
    # Elasticity: Gradients of J w.r.t F
    g_J0 = wp.cross(f1, n_hat)
    g_J1 = wp.cross(n_hat, f0)

    # Elasticity: First Piola-Kirchhoff Stress (PK1)
    # P = mu * F + (lambda * (J - 1) - mu) * dJ/dF
    alpha = lmbd * (J - 1.0) - mu

    PK1_col0 = mu * f0 + alpha * g_J0
    PK1_col1 = mu * f1 + alpha * g_J1

    # Elasticity: Map PK1 to Nodal Forces (Project dF/dx)
    mask0 = float(v_order == 0)
    mask1 = float(v_order == 1)
    mask2 = float(v_order == 2)

    df0_dx_s = DmInv00 * (mask1 - mask0) + DmInv10 * (mask2 - mask0)
    df1_dx_s = DmInv01 * (mask1 - mask0) + DmInv11 * (mask2 - mask0)

    force = -(PK1_col0 * df0_dx_s + PK1_col1 * df1_dx_s)

    # Elasticity: Hessian
    I33 = wp.identity(n=3, dtype=float)
    
    # A. Geometric Stiffness (Hessian of J)
    Pn = I33 - wp.outer(n_hat, n_hat)
    
    skew_f0 = wp.skew(f0)
    skew_f1 = wp.skew(f1)
    skew_n  = wp.skew(n_hat)
    
    Pn_Sf0 = Pn * skew_f0
    Pn_Sf1 = Pn * skew_f1

    H_J00 = -inv_J * skew_f1 * Pn_Sf1
    H_J11 = -inv_J * skew_f0 * Pn_Sf0
    H_J01 = -skew_n + inv_J * skew_f1 * Pn_Sf0
    
    # B. Material Stiffness Blocks
    H_00 = mu * I33 + lmbd * wp.outer(g_J0, g_J0) + alpha * H_J00
    H_11 = mu * I33 + lmbd * wp.outer(g_J1, g_J1) + alpha * H_J11
    H_01 = lmbd * wp.outer(g_J0, g_J1) + alpha * H_J01
    
    # C. Project to Vertex Hessian
    term_00 = df0_dx_s * df0_dx_s
    term_11 = df1_dx_s * df1_dx_s
    term_01 = df0_dx_s * df1_dx_s
    
    hessian = term_00 * H_00 + term_11 * H_11 + term_01 * (H_01 + wp.transpose(H_01))

    # -------------------------------------------------------------
    # DAMPING: Green Strain (StVK-style)
    # We use StVK damping formulation here to avoid shrinking artifacts.
    # -------------------------------------------------------------
    if damping > 0.0:
        inv_dt = 1.0 / dt
        
        # Compute Green Strain G = 0.5 * (F^T F - I)
        # Reuse dot products from F
        f0_dot_f0 = wp.dot(f0, f0)
        f1_dot_f1 = wp.dot(f1, f1)
        f0_dot_f1 = wp.dot(f0, f1)
        ...
        # Frobenius norm squared of Green strain
        G_frobenius_sq = G00 * G00 + G11 * G11 + 2.0 * G01 * G01

        # Only apply damping if there is deformation to avoid singularities
        if G_frobenius_sq > 1.0e-20:
            # Calculate Velocity of F
            x0_prev = pos_prev[v0]
            x01_prev = pos_prev[v1] - x0_prev
            x02_prev = pos_prev[v2] - x0_prev
            ...
            # dCmu/dt
            dCmu_dt = G00_n * dG_dt_00 + G11_n * dG_dt_11 + 2.0 * G01_n * dG_dt_01
            
            # dCmu/dF (Gradients w.r.t F vectors)
            dCmu_df0 = G00_n * f0 + G01_n * f1
            dCmu_df1 = G01_n * f0 + G11_n * f1
            
            # dCmu/dx (Project to vertex)
            dCmu_dx = dCmu_df0 * df0_dx_s + dCmu_df1 * df1_dx_s
            
            kd_mu = mu * damping
            force += -kd_mu * dCmu_dt * dCmu_dx
            hessian += kd_mu * inv_dt * wp.outer(dCmu_dx, dCmu_dx)

            # --- Damping Constraint 2: Area/Volume tr(G) ---
            # dClmbd/dt
            dClmbd_dt = dG_dt_00 + dG_dt_11
            
            # dClmbd/dF = F
            # dClmbd/dx
            dClmbd_dx = f0 * df0_dx_s + f1 * df1_dx_s
...
def evaluate_body_particle_contact(
    ...
    if penetration_depth > 0:
        # Non-linear stiffening: F = k * d * (1 + 100 * d)
        # This provides much stronger contact force as penetration increases
        stiffness_scale = 1.0 #+ 100.0 * penetration_depth
        
        body_contact_force_norm = penetration_depth * soft_contact_ke * stiffness_scale
        body_contact_force = n * body_contact_force_norm
        # Approximate Hessian update
        body_contact_hessian = soft_contact_ke * stiffness_scale * wp.outer(n, n)
...
def solve_trimesh_no_self_contact_tile(
    ...
    while batch_counter + thread_idx < num_adj_faces:
        ...
        # Use simplified isotropic SNH for better stability and natural wrinkling
        # When paired with AVBD edge constraints:
        # - AVBD locks maximum edge length (prevents stretching)
        # - Isotropic SNH with low mu allows shape deformation (enables shearing/wrinkling)
        f_tri, h_tri = evaluate_isotropic_snh(
            tri_index,
            vertex_order,
            pos,
            pos_prev,
            ...
            tri_poses[tri_index],
            tri_areas[tri_index],
            tri_materials[tri_index, 0],  # mu: shear modulus (recommend small: 20-100)
            tri_materials[tri_index, 1],  # lambda: area modulus (recommend large: 1000+)
            tri_materials[tri_index, 2],  # damping
            dt,
        )
        f += f_tri
        h += h_tri
...
def solve_trimesh_no_self_contact(
    ...
    for i_adj_tri in range(get_vertex_num_adjacent_faces(adjacency, particle_index)):
        ...
        f_tri, h_tri = evaluate_isotropic_snh(
            ...
            tri_poses[tri_id],
            tri_areas[tri_id],
            tri_materials[tri_id, 0],  # mu: shear modulus (recommend small: 20-100)
            tri_materials[tri_id, 1],  # lambda: area modulus (recommend large: 1000+)
            tri_materials[tri_id, 2],  # damping
            dt,
        )
        f = f + f_tri
        h = h + h_tri
...
@wp.func
def evaluate_avbd_edge_constraint(
    particle_idx: int,  # Current particle index being computed
    idx_a: int,  # First particle index of the constraint
    idx_b: int,  # Second particle index of the constraint
    pos: wp.array(dtype=wp.vec3),
    pos_prev: wp.array(dtype=wp.vec3),
    rest_length: float,
    limit_ratio: float,
    # AVBD state variables (current iteration values)
    stiffness: float,
    lam: float,  # Lambda (Lagrange multiplier)
):
    """
    AVBD (Augmented Vertex Block Descent) edge constraint for hard stretch limits.
    
    This is a ONE-SIDED constraint: only prevents stretching (elongation), NOT compression.
    Allowing compression is KEY for cloth to form natural wrinkles.
    
    Reference: AVBD SIGGRAPH 2025
    
    Args:
        particle_idx: Current vertex being solved
        idx_a, idx_b: The two vertices of the edge
        pos, pos_prev: Current and previous positions
        rest_length: Rest length of the edge
        limit_ratio: Maximum stretch ratio (e.g., 0.005 for 0.5%)
        stiffness: Current AVBD stiffness (adaptive)
        lam: Current Lagrange multiplier (accumulated)
    """
    diff = pos[idx_a] - pos[idx_b]
    l = wp.length(diff)

    if l < 1e-9:
        return wp.vec3(0.0), wp.mat33(0.0)

    # 1. Hard constraint: L <= L_max (one-sided, only prevent stretching)
    l_max = rest_length * (1.0 + limit_ratio)
    C = l - l_max

    # 2. Explosive Error Correction
    # Smooth out residual error from previous frame to prevent violent bouncing
    l_prev = wp.length(pos_prev[idx_a] - pos_prev[idx_b])
    C_prev = l_prev - l_max
    
    bias = 0.0
    if C_prev > 0.0:
        bias = AVBD_ALPHA * C_prev
    C_eff = C - bias

    # 3. Compute augmented force
    lam_total = lam + stiffness * C_eff

    # KEY LOGIC: One-sided constraint!
    # Only apply force when cloth tries to stretch (lam_total > 0).
    # Allow cloth to compress (shorten), leaving room for SNH to simulate shear.
    if lam_total <= 0.0:
        return wp.vec3(0.0), wp.mat33(0.0)

    dir = diff / l

    # 4. Compute force direction based on which vertex we're solving
    # If current particle is idx_a, force pulls back (opposite to diff direction)
    # If current particle is idx_b, force pushes forward (along diff direction)
    sign = -1.0
    if particle_idx == idx_b:
        sign = 1.0
    force = sign * lam_total * dir

    # 5. Stable Hessian (Geometric Stiffness Projection)
    # Stiffness term: k * n * n^T (always SPD)
    H_s = stiffness * wp.outer(dir, dir)

    # Diagonalized geometric stiffness term for guaranteed SPD Hessian
    # This is the secret to VBD not exploding under high stiffness
    geo = lam_total / l
    gx = geo * (1.0 - dir[0] * dir[0])
    gy = geo * (1.0 - dir[1] * dir[1])
    gz = geo * (1.0 - dir[2] * dir[2])
    H_g = wp.diag(wp.vec3(gx, gy, gz))

    return force, H_s + H_g
...
def evaluate_spring_force_and_hessian(
    ...
    limit = spring_control[spring_idx]

    if limit > 0.0:
        l_limit = l0 * (1.0 + limit)
        if l <= l_limit:
            return wp.vec3(0.0), wp.mat33(0.0)
        l0 = l_limit
...
@wp.kernel
def accumulate_spring_force_and_hessian_avbd(
    # inputs
    dt: float,
    current_color: int,
    pos_prev: wp.array(dtype=wp.vec3),
    ...
    spring_control: wp.array(dtype=float),
    # AVBD state arrays
    avbd_lambdas: wp.array(dtype=float),
    avbd_stiffnesses: wp.array(dtype=float),
    # outputs: particle force and hessian
    particle_forces: wp.array(dtype=wp.vec3),
    particle_hessians: wp.array(dtype=wp.mat33),
):
    """
    Accumulate spring forces and Hessians with AVBD strain limit handling.
    Uses Augmented Lagrangian method for better convergence on strain limit constraints.
    """
    t_id = wp.tid()
    ...
    for spring_counter in range(num_adj_springs):
        ...
        v0 = spring_indices[spring_index * 2]
        v1 = spring_indices[spring_index * 2 + 1]
        limit = spring_control[spring_index]

        # Check if this is a strain limit constraint (limit > 0)
        if limit > 0.0:
            # Use AVBD approach for strain limit (one-sided hard constraint)
            spring_force, spring_hessian = evaluate_avbd_edge_constraint(
                particle_index,
                v0,
                v1,
                pos,
                ...
                limit,
                avbd_stiffnesses[spring_index],
                avbd_lambdas[spring_index],
            )
        else:
            # Use standard spring evaluation for regular springs
            spring_force, spring_hessian = evaluate_spring_force_and_hessian(
                particle_index,
                spring_index,
                dt,
...
@wp.kernel
def update_avbd_strain_limit_duals(
    pos: wp.array(dtype=wp.vec3),
    pos_prev: wp.array(dtype=wp.vec3),
    spring_indices: wp.array(dtype=int),
    spring_rest_length: wp.array(dtype=float),
    spring_control: wp.array(dtype=float),
    # AVBD state arrays (read/write)
    avbd_lambdas: wp.array(dtype=float),
    avbd_stiffnesses: wp.array(dtype=float),
    max_stiffness: float,
):
    """
    Update AVBD dual variables (lambda and stiffness) after VBD iteration.
    This kernel should be called after all position updates are complete.
    """
    tid = wp.tid()

    # Check if strain limit is enabled for this spring
    limit_ratio = spring_control[tid]
    if limit_ratio <= 0.0:
        return

    idx_a = spring_indices[tid * 2]
    idx_b = spring_indices[tid * 2 + 1]

    # Recompute effective error C_eff with regularization
    # Must maintain consistency with evaluate_avbd_edge_constraint
    l = wp.length(pos[idx_a] - pos[idx_b])
    l_max = spring_rest_length[tid] * (1.0 + limit_ratio)
    C = l - l_max
    ...
    k = avbd_stiffnesses[tid]
    lam = avbd_lambdas[tid]

    # Update Lambda (Dual Update, Eq 11)
    lam_new = lam + k * C_eff

    # Inequality clamping
    if lam_new <= 0.0:
        lam_new = 0.0
        # Don't increase stiffness when constraint is not active
    else:
        # Update stiffness (Stiffness Update, Eq 12)
        # Only increase stiffness when constraint is active and violated
        k = k + AVBD_BETA * wp.abs(C_eff)
        if k > max_stiffness:
            k = max_stiffness

    avbd_lambdas[tid] = lam_new
    avbd_stiffnesses[tid] = k


@wp.kernel
def decay_avbd_state(
    avbd_lambdas: wp.array(dtype=float),
    avbd_stiffnesses: wp.array(dtype=float),
    spring_control: wp.array(dtype=float),
    lambda_decay: float,
    stiffness_decay: float,
    min_stiffness: float,
):
    """
    Apply decay to AVBD state variables at the start of each frame.
    This provides warm start and improves continuity between frames.
    """
    tid = wp.tid()

    # Only decay for strain limit springs
    if spring_control[tid] <= 0.0:
        return

    # Decay lambda
    avbd_lambdas[tid] = avbd_lambdas[tid] * lambda_decay

    # Decay stiffness but maintain minimum
    k = avbd_stiffnesses[tid] * stiffness_decay
    if k < min_stiffness:
        k = min_stiffness
    avbd_stiffnesses[tid] = k
...
class SolverVBD(SolverBase):
    ...
    def __init__(
        ...
        # AVBD state variables for strain limit constraints
        # Lambda (Lagrange multiplier) for each spring - accumulates over iterations
        self.avbd_lambdas = wp.zeros(model.spring_count, dtype=float, device=self.device)
        # Adaptive stiffness for each spring - starts small and grows as needed
        self.avbd_stiffnesses = wp.full(
            model.spring_count, AVBD_INITIAL_STIFFNESS, dtype=float, device=self.device
        )
    ...
    def simulate_one_step_no_self_contact(
        ...
        # AVBD warm start: decay lambda and stiffness at the start of each frame
        if model.spring_count:
            wp.launch(
                kernel=decay_avbd_state,
                inputs=[
                    self.avbd_lambdas,
                    self.avbd_stiffnesses,
                    self.model.spring_control,
                    0.99,  # lambda_decay
                    0.99,  # stiffness_decay
                    AVBD_INITIAL_STIFFNESS,  # min_stiffness
                ],
        ...
        for _iter in range(self.iterations):
            ...
            for color in range(len(self.model.particle_color_groups)):
                ...
                if model.spring_count:
                    wp.launch(
                        kernel=accumulate_spring_force_and_hessian_avbd,
                        inputs=[
                            dt,
                            color,
                            self.particle_q_prev,
            ...
            # AVBD dual update: update lambda and stiffness after all positions are updated
            if model.spring_count:
                wp.launch(
                    kernel=update_avbd_strain_limit_duals,
                    inputs=[
                        state_in.particle_q,
                        self.particle_q_prev,
                        self.model.spring_indices,
