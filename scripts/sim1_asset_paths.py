...
Override with environment variable:
  ...
  SIM1_ASSETS_ROOT   Absolute or relative path to the folder that contains
                     acone/, cloth/, random/, model/, etc. (the same directory
                     used as --local-dir by HF download tools).
...
_SIM1_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_ASSETS_ENV = "SIM1_ASSETS_ROOT"


def get_project_root() -> str:
    """SIM1 repository root directory."""
    return _SIM1_ROOT
...
def get_flow_ckpt_three_path() -> str:
    """Diffusion policy checkpoint shipped with the HF bundle (assets/model/flow_ckpt_three.pth)."""
