"""
SIM1-DataGen: Data generation entry point.

Generates trajectory data using the SIM1 data generation pipeline.
Supports both baseline and diffusion-policy (DP) based generation modes.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="SIM1-DataGen: trajectory data generation")
    parser.add_argument(
        "--data_folder",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of trajectories to generate (default: 2)",
    )
    parser.add_argument(
        "--use_dp",
        action="store_true",
        help="Use diffusion-policy (DP) based selector/splitter",
    )
    parser.add_argument(
        "--mode",
        type=str,
        ...
        help="Generation mode: 'fine' (SplitterFine+SelectorFine) or 'baseline' (Splitter+Selector)",
    )
    parser.add_argument(
        "--task_split_cfg",
        type=str,
        default=None,
        help="Path to task split config JSON (required for baseline mode)",
    )
...
def main():
    ...
    datagen = DataGenerator(env, splitter, selector, data_folder, use_dp=args.use_dp)
