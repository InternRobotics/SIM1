...
def process_single_video(video_path, sample_rate, transform, model, device, counter, lock):
    """
    Worker: decode + run one video through the model
    
    Args:
        video_path: path to video file
        sample_rate: frame subsampling
        transform: torchvision transform
        model: classifier
        device: torch device
        counter: ThreadSafeCounter
        lock: serialize GPU forward passes
    
    Returns:
        (basename, prob, pred, error): outputs / error string
    """
    basename = os.path.splitext(os.path.basename(video_path))[0]
    
    try:
        # Decode frames on CPU (parallel-safe)
        frames = load_video_frames(video_path, sample_rate)
        frames = torch.stack([transform(f) for f in frames])
        
        # GPU forward under lock
        with lock:
        ...
        if pred:
            ...
            print(f"{basename} → SUCCESS/{count}, prob={prob:.4f}")
        else:
            count = counter.increment_failure()
            print(f"{basename} → FAILED/{count}, prob={prob:.4f}")
        
        return basename, prob, pred, None
        
    except Exception as e:
        count = counter.increment_error()
        print(f"{basename} → ERROR/{count}: {str(e)}")
        return basename, "error", None, str(e)


def inference(model, root, sample_rate, device, num_threads=4):
    """
    Run batched inference over a folder of videos
    
    Args:
        model: trained checkpoint
        root: dataset root (expects videos/ under it)
        sample_rate: frame subsampling stride
        device: torch device
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Processing videos"):
            basename, prob, pred, error = future.result()
            output[basename] = prob
    
    print(f"\n{'='*60}")
    print(f"Summary: SUCCESS={counter.suc}, FAILED={counter.fail}, ERROR={counter.err}")
    print(f"{'='*60}\n")
